...
//            self.tableView.reloadData()
    ...
    private func loadAllOpenedSessions() {
        ...
        query.observeSingleEvent(of: .value, with: {(snapshot) in for childSnapshot in snapshot.children {
            ...
            if ended == false && self.currentVolunteer == volunteerId {
                ...
                guard let session = VolunteerSessionLogHistory(date: date, requestedCall: requestedCall, seekerId: seekerId, seekerName: seekerName, seekerRating: seekerRating, sessionID: id, volunteerID: volunteerId, volunteerName: volunteerName, volunteerRating: volunteerRating, ended: ended) else {
                    fatalError("Unable to instantiate this material")
                }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? VolunteerRequestedSeekerTableViewCell else {
            fatalError("The dequeued cell is not an instance of ReadingMaterialTableViewCell.")
        }
        
//        let session = self.sessions[indexPath.row]
        
        var rowSeeker: VolunteerSeeker?
